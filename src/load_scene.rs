...
let entit= commands.spawn((
        ...
        Transform::from_translation(Vec3 { x: 529. + x as f32 *width as f32*50 as f32, y: -130. + y as f32 *height as f32*50 as f32, z: -32. }).with_scale(Vec3 { x: width, y: height, z: 2.}),
        MeshMaterial2d(materials.add(ColorMaterial::from_color(Srgba::rgb(0.1*x as f32, 1.0, 0.0)))),
        Load,Code{ s: EnumCode::MyKey((String::from(format!("[Text(\"{}\")]",(x+y*h)))))},
    )).id();        let regular_font_handle: Handle<Font> = Default::default();
commands.spawn((
    Text2d::new(format!("{}",(x+y*h))),
    TextColor(Color::WHITE),
    TextFont::from(regular_font_handle.clone()).with_font_size(28.),
